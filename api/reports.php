 ...
 * GET /api/reports.php?tipo=sal&commessa_id=N
 * GET /api/reports.php?tipo=costi&commessa_id=N
 * GET /api/reports.php?tipo=scadenze
...
function reportAvanzamento(?int $commessaId): never
{
    if ($commessaId) {
        ...
        $perFase = Database::fetchAll(
            'SELECT f.nome AS fase, f.colore,
             ...
             FROM fasi_lavoro f
             LEFT JOIN tasks t ON t.fase_id = f.id AND t.commessa_id = :id
        ...
        jsonResponse([
            ...
            'per_fase'   => $perFase,
            'riepilogo'  => [
                'totale_tasks'   => count($tasks),
                'completati'     => count(array_filter($tasks, fn($t) => $t['stato'] === 'COMPLETATO')),
                'in_ritardo'     => count(array_filter($tasks, fn($t) => $t['stato'] === 'IN_RITARDO')),
    ...
    // Report globale tutte le commesse
    ...
    jsonResponse([
        ...
        'commesse'  => $commesse,
        'totali'    => [
            'n_commesse'       => count($commesse),
            'in_esecuzione'    => count(array_filter($commesse, fn($c) => $c['stato'] === 'IN_ESECUZIONE')),
            'completate'       => count(array_filter($commesse, fn($c) => $c['stato'] === 'COMPLETATA')),
            'valore_totale'    => formatEuro(array_sum(array_column($commesse, 'importo_contrattuale'))),
        ],
    ]);
}
...
function reportSal(int $commessaId): never
{
    ...
    $totPagato   = array_sum(array_map(fn($s) => $s['stato'] === 'PAGATO' ? (float)$s['importo_netto'] : 0, $sal));
    $totApprovato = array_sum(array_map(fn($s) => in_array($s['stato'],['APPROVATO','PAGATO']) ? (float)$s['importo_totale'] : 0, $sal));
    $commessa    = Database::fetchOne('SELECT importo_contrattuale, importo_sicurezza FROM commesse WHERE id = :id', [':id' => $commessaId]);
...
function reportCosti(int $commessaId): never
{
    $categorie = Database::fetchAll(
        'SELECT cl.codice, cl.descrizione, cl.unita_misura, cl.prezzo_unitario,
         ...
         FROM categorie_lavoro cl
         LEFT JOIN sal_voci sv ON sv.categoria_id = cl.id
             AND sv.sal_id IN (SELECT id FROM sal WHERE commessa_id = :cid AND stato IN ("APPROVATO","PAGATO"))
...
function reportScadenze(?int $commessaId): never
{
    ...
    $scadenze = Database::fetchAll($sql, $params);
    $scadenze = array_map(function($s) {
    ...
    jsonResponse([
        ...
        'scadenze'   => $scadenze,
        'riepilogo'  => [
            'totale'  => count($scadenze),
            'scadute' => count(array_filter($scadenze, fn($s) => $s['giorni'] < 0)),
            'critiche' => count(array_filter($scadenze, fn($s) => $s['giorni'] >= 0 && $s['giorni'] <= 7)),
        ],
    ]);
}
...
function reportGantt(int $commessaId): never
{
    ...
    $commessa = Database::fetchOne('SELECT id, codice_commessa, oggetto, data_inizio_prevista, data_fine_prevista FROM commesse WHERE id = :id', [':id' => $commessaId]);
    if (!$commessa) jsonError('Commessa non trovata', 404);
    ...
    $dipendenze = Database::fetchAll(
        'SELECT dt.task_id, dt.task_pred_id, dt.tipo AS dep_tipo, dt.lag_giorni
         ...
         JOIN tasks t ON t.id = dt.task_id WHERE t.commessa_id = :id',
        [':id' => $commessaId]
    );
...
function reportRiepilogoGlobale(): never
{
    $stats = Database::fetchOne(
        'SELECT
         ...
         FROM commesse WHERE stato != "ANNULLATA"'
    );
